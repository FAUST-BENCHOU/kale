...
def main():
    ...
    parser = argparse.ArgumentParser(
        description=ARGS_DESC, formatter_class=RawTextHelpFormatter
    )
    ...
    metadata_group = parser.add_argument_group(
        "Notebook Metadata Overrides", METADATA_GROUP_DESC
    )
    ...
    metadata_group.add_argument(
        "--storage-class-name",
        type=str,
        help="The storage class name for the created volumes",
    )
    metadata_group.add_argument(
    ...
    mt_overrides_group = next(
        filter(
            lambda x: x.title == "Notebook Metadata Overrides", parser._action_groups
        )
    )
