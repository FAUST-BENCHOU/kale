...
def upload_pipeline(
    ...
    if not pipeline_id:
        ...
        client.delete_pipeline_version(
            pipeline_id=pipeline_id, pipeline_version_id=delete_vid
        )
        log.info(
            "Deleted pipeline version with name '%s' and ID: %s",
            pipeline_name,
            upp.pipeline_id,
        )
    else:
    ...
    log.info(
        "Successfully uploaded version '%s' for pipeline '%s'.",
        version_name,
        pipeline_name,
    )
...
def run_pipeline(
    ...
    **kwargs,
    ...
    if not run_name:
        run_name = "%s-%s-%s" % (pipeline_name, version_name, utils.random_string())
...
def get_current_uimetadata(
    uimetadata_path=KFP_UI_METADATA_FILE_PATH, default_if_not_exist=False
):
    ...
    for name, value in metrics.items():
        if not isinstance(value, (int, float)):
            try:
                ...
                print(
                    "Variable {} with type {} not supported as pipeline"
