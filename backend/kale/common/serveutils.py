...
from kale.common import astutils, flakeutils, podutils, k8sutils, jputils, utils
...
class KFServer(object):
    ...
    def __repr__(self):
        ...
        if utils.is_ipython():
            ...
            html = (
                "InferenceService <pre>%s</pre> serving requests at host"
                " <pre>%s</pre><br>"
    ...
    def _to_tensor(self, data):
        log.warning(
            "Kale does not yet support converting the predicted response to a tensor."
        )
        return data
...
def create_inference_service(
    ...
    if predictor not in PREDICTORS:
        raise ValueError(
            "Invalid predictor: %s. Choose one of %s" % (predictor, PREDICTORS)
        )
    ...
    if predictor == "custom":
        ...
        if image is not None:
            log.info(
                "Creating an InferenceService with predictor '%s'. Ignoring image...",
                predictor,
            )
        if port is not None:
            log.info(
                "Creating an InferenceService with predictor '%s'. Ignoring port...",
                predictor,
            )
...
def _get_runtime_version(predictor: str):
    ...
    if not library:
        raise ValueError(
            "The provided predictor is not backed by any Kale marshalling backend."
        )
    if len(library) > 1:
        raise ValueError(
            "Too many backends are matching the '%s' predictor:"
            " %s" % (predictor, library)
        )
...
def _add_owner_references(infs_name: str, pvc_name: str):
    ...
    log.info(
        "Adding owner references to PVC '%s' for InferenceService '%s'",
        pvc_name,
        infs_name,
    )
...
def get_inference_service(name: str):
    ...
    return k8s_co_client.get_namespaced_custom_object(
        CO_GROUP, CO_VERSION, ns, CO_PLURAL, name
    )
