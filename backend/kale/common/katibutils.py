...
TRIAL_PARAMETERS_BASE = [
    {"name": KALE_PARAM_TRIAL_NAME, "reference": "${trialSpec.Name}"}
]
    ...
    **kwargs,
    ...
    try:
        ...
        annotate_trial(
            run_name,
            pod_namespace,
            {KALE_KATIB_KFP_ANNOTATION_KEY: run_id},
            api_version,
        )
    except Exception:
        log.exception(
            "Failed to annotate Trial '%s' with the KFP Run UUID '%s'", run_name, run_id
        )

    log.info("Getting Workflow name for run '%s'...", run_id)
    workflow_name = kfputils.get_workflow_from_run(kfputils.get_run(run_id))[
        "metadata"
    ]["name"]
    ...
    log.info(
        "Trial name: %s, UID: %s", trial["metadata"]["name"], trial["metadata"]["uid"]
    )
    ...
    try:
        ...
        log.exception(
            "Failed to annotate Workflow '%s' with the Katib details", workflow_name
        )
...
def get_owner_experiment_from_trial(trial):
    ...
    owner = _get_owner_experiment(
        trial["metadata"]["ownerReferences"], api_version=api_version
    )
    if not owner:
...
def _construct_experiment_cr_v1beta1(
    ...
    trial_tmpl["trialParameters"].extend(
        [{"name": p, "reference": p} for p in param_names]
    )
    ...
    experiment = {
        "apiVersion": "%s/%s" % (KATIB_API_GROUP, KATIB_API_VERSION_V1BETA1),
        "kind": "Experiment",
        "metadata": {"name": name},
        "spec": spec,
    }
