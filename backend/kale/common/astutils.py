...
def parse_assignments_expressions(code):
    ...
    for block in tree.body:
        if not isinstance(block, ast.Assign):
            raise ValueError(
                "Must provide just primitive types assignments in variables block"
            )
        ...
        if (
            ...
            raise ValueError(
                "Must provide single variable assignments in variables block"
            )
        target = targets[0].id
        value = block.value
        ...
        if isinstance(value, ast.Num):  # [int|float]
            ...
            if value is None:
                raise ValueError(
                    "`None` value None is not supported in pipeline parameters"
                )
            var_type = "bool"
        else:
            raise ValueError(
...
def parse_metrics_print_statements(code):
    ...
    match_print = re.compile(r"^print\(%s\)$" % var_name_pattern, re.MULTILINE)
...
def get_function_source(fn: Callable, strip_signature=True) -> str:
    ...
    if not isinstance(fn, types.FunctionType):
        raise TypeError(
            "Input function is not of type FunctionType. Found type: %s" % type(fn)
        )
    ...
    if source[0] == "@":
        ...
        idx = next(
            (
                _idx
                for _idx, line in enumerate(source.splitlines())
...
def link_fns_to_inputs_vars(code: str) -> Dict[str, List[str]]:
    ...
    for node in tree.body:
        ...
        fn_to_args[fn_name] = [
            name_node.id
            for name_node in func_node.args
    ...
    for node in tree.body:
        ...
        for target in node.targets:
            if isinstance(target, ast.Tuple):
                ...
                raise RuntimeError("Ast node '%s' not supported" % type(target))
