...
def generate_html_output(outputs):
    ...
    if not isinstance(outputs, list):
        raise ValueError(
            "A notebook's cell outputs must be a valid list. Found {} instead.".format(
                type(outputs)
            )
        )
    html_body = ""
    ...
    for o in outputs:
        ...
        if not output_type:
            raise ValueError(
                "Cell output dict has not `output_type` field. Output: {}".format(o)
            )
...
async def capture_streams(kc, exit_on_error=False):
    ...
    while True:
        try:
            ...
            print(
                "The Kale kernel stream watcher thread raised an Empty"
                " exception, exiting..."
            )
            return
        ...
        if msg_type == "stream":  # stdout or stderr
            if content["name"] == "stdout":
                ...
                raise NotImplementedError(
                    "stream message content name not recognized: {}".format(
                        content["name"]
                    )
                )
        if msg_type == "error":  # error and exceptions
            ...
            if content["ename"] == KaleGracefulExit.__name__:
                log.error(
                    "Received a %s exception. Exiting..." % KaleGracefulExit.__name__
                )
...
def run_code(source: tuple, kernel_name="python3"):
    ...
    if pkg_version.parse(IPython.__version__) < pkg_version.parse("7.6.0"):
        raise RuntimeError(
            "IPython version {} not supported."
            " Kale requires at least version 7.6.0.".format(IPython.__version__)
        )
...
def get_notebook_path():
    ...
    for srv in serverapp.list_running_servers():
        ...
        if req.status_code != 200:
            log.error("Session request failed with status code %s" % req.status_code)
            return None
        try:
