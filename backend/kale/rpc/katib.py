...
def _sanitize_parameters(
    request, parameters, parameter_names, defaults, parameters_type
):
    ...
    for param in parameter_names:
        if param not in parameters and param not in defaults:
            request.log.exception(
                "%s parameter '%s' was not provided", parameters_type, param
            )
            raise RPCNotFoundError(
                details=("%s parameter '%s' is required" % (parameters_type, param)),
                trans_id=request.trans_id,
            )
...
def _sanitize_katib_spec(request, katib_spec):
    ...
    return _sanitize_parameters(
        request, katib_spec, KATIB_PARAMETER_NAMES, KATIB_DEFAULTS, "Katib"
    )
...
def create_katib_experiment(
    request, pipeline_id, version_id, pipeline_metadata, output_path
):
...
def get_experiment(request, experiment, namespace):
    ...
    try:
        exp = k8s_co_client.get_namespaced_custom_object(
            co_group,
            katibutils.discover_katib_version(),
            namespace,
            co_plural,
            experiment,
        )
        ...
        raise RPCUnhandledError(
            message="Failed to get Katib experiment",
            details=str(e),
            trans_id=request.trans_id,
        )
