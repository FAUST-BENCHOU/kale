...
def import_func(request, import_func_str):
    ...
    try:
        ...
        request.log.exception(
            "Exception while importing function '%s' of module '%s': %s",
            func_str,
            mod_str,
            e,
        )
        raise ImportError(
            "Function `%s' of module `%s' cannot be found: %s" % (func_str, mod_str, e)
        )
...
def format_success(result, trans_id):
    ...
    return utils.serialize(
        {"code": errors.Code.OK.value, "result": result, "trans_id": trans_id}
    )
...
def run(func, kwargs, ctx):
    ...
    try:
        ...
        return errors.RPCImportError(
            message=str(e), trans_id=request.trans_id
        ).serialize()
    ...
    try:
        ...
        exc_info = sys.exc_info()
        request.log.exception(
            ("RPC function '%s' raised an unhandled exception"), func.__name__
        )
